...
var PlandEnabledCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		err = c.EnablePlan(context.Background(), teamID, compositeID, planID)
		if err != nil {
			fmt.Fprintf(os.Stderr, err.Error())
			os.Exit(-1)
		}
...
func init() {
	...
	PlandEnabledCmd.Flags().StringVarP(&planID, content.CMD_FLAG_ID_LONG, content.CMD_FLAG_ID_SHORT, "", content.CMD_FLAG_PLANID_DESCRIPTION)
	PlandEnabledCmd.Flags().StringVarP(&comositeID, content.CMD_FLAG_ID_LONG, content.CMD_FLAG_ID_SHORT, "", content.CMD_FLAG_COMPOSITE_DESCRIPTION)
}
